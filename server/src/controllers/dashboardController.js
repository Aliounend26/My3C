...
export const getStudentDashboard = asyncHandler(async (req, res) => {
  ...
  const [courses, upcomingCourses, sections, resources, quizzes, quizResults, announcements, messages, videos, notifications, courseProgressDocs, sectionProgressDocs] = await Promise.all([
    ...
    StudentCourseProgress.find({ student: req.user._id, course: { $in: courseIds } }),
    StudentSectionProgress.find({ student: req.user._id, course: { $in: courseIds } })
  ]);
  ...
  const courseProgressMap = new Map(courseProgressDocs.map((doc) => [doc.course.toString(), doc]));
  const completedSectionsByCourse = sectionProgressDocs.reduce((acc, doc) => {
    const key = doc.course.toString();
    if (!acc.has(key)) {
      acc.set(key, 0);
    }
    if (doc.isCompleted) {
  ...
  const courseCards = courses.map((course) => {
    ...
    const storedCourseProgress = courseProgressMap.get(courseId);
    const completedSections = completedSectionsByCourse.get(courseId) || 0;
    const fallbackProgress = courseSections.length ? Math.round((completedSections / courseSections.length) * 100) : 0;
    const progress = storedCourseProgress?.progressPercent ?? fallbackProgress;
