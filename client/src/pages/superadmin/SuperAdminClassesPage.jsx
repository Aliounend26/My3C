...
export const SuperAdminClassesPage = () => {
  ...
  const openEdit = (row) => {
    ...
    setForm({
      name: row.name || "",
      code: row.code || "",
      formation: row.formation?._id || row.formation || "",
      teacher: row.teacher?._id || row.teacher || "",
      students: (row.students || []).map((student) => student._id || student)
    });
    setOpen(true);
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Organisation academique"
        title="Gestion des classes"
        description="Structurez les classes, rattachez-les aux formations et affectez les formateurs ainsi que les etudiants."
        actions={
              ...
              render: (row) => (
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      await resourceService.delete(`/classes/${row._id}`);
                      await load();
                    }}
                  >
                    Supprimer
      ...
      <Modal
        open={open}
        title={editing ? "Modifier une classe" : "Creer une classe"}
        onClose={() => {
          setOpen(false);
          setEditing(null);
          setForm(initialForm);
        ...
        <form className="grid gap-4 md:grid-cols-2" onSubmit={submit}>
          ...
          <label className="md:col-span-2">
            ...
            <select
              ...
              multiple
              value={form.students}
              onChange={(event) => setForm((current) => ({ ...current, students: Array.from(event.target.selectedOptions, (option) => option.value) }))}
            >
          ...
          <button className="rounded-2xl bg-slate-950 px-4 py-3 text-sm font-semibold text-white md:col-span-2">
            {editing ? "Enregistrer les modifications" : "Creer la classe"}
          </button>
        </form>
      </Modal>
    </div>
